...
public class MainClass extends JFrame
{
	...
	public MainClass()
	{
		...
		add(bg);
		bg.setLayout(null);
		board.setBound(100,120,400,400);
		board.setBorder(BorderFactory.createLineBorder(Color.red3));
		bg.add(board);
		...
		bg.add(Va.reset);
		Va.reset.addActionListener(NewGameListener());
		addButtons();
		setVisible(true);
	}
	...
	class TicListener implements ActionListener
	{
		public void actionPerformed(ActionEvent evt)
		{
			...
			if(Va.user==0)
				bc.setIcon(new ImageIcon(getClass().getResource("images/user1.png")));
			
			else
				bc.setIcon(new ImageIcon(getClass().getResource("images/user2.png")));
		...
		void checkWinner()
		{
			for(int i = 0;i<3; i++)
			{
				...
				{
					Va.bt[i][0].setBaclground(Color.gray);
					Va.bt[i][1].setBaclground(Color.gray);
					Va.bt[i][2].setBaclground(Color.gray);
					return;
				}
			}
			...
			for(int i = 0; i<3; i++)
			{
				...
				if(Va.user1 == 1 || Va.user2 == 1)
				{
					Va.bt[0][i].setBaclground(Color.gray);
					Va.bt[1][i].setBaclground(Color.gray);
					Va.bt[2][i].setBaclground(Color.gray);
					return;
				}
			}
			...
			if(Va.user1 == 1 || Va.user2 == 1)
				{
					Va.bt[0][0].setBaclground(Color.gray);
					Va.bt[1][1].setBaclground(Color.gray);
					Va.bt[2][2].setBaclground(Color.gray);
					return;
				}
				//----------------
			...
			if(Va.user1 == 1 || Va.user2 == 1)
				{
					Va.bt[0][2].setBaclground(Color.gray);
					Va.bt[1][1].setBaclground(Color.gray);
					Va.bt[2][0].setBaclground(Color.gray);
					return;
				}
		}
